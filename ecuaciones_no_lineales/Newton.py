...
class Newton:
    ...
    def algoritmo(self, xi, Function,iterations, tolerance, err_type):
        print(err_type)
        xi_1 = xi
        counter = 1
        error = tolerance+1
        ...
        if(fx == 0):
            ...
            self.raiz=f"[{xi} raiz aproximada]"
        elif(dfx == 0):
            self.raiz=f"[{xi}posible raiz]"
        else:
            self.raiz=f"Se excedió interacion"
